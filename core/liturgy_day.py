...
- ``DE`` / ``EN`` / ``ES`` / ``IT`` / ``PT`` → Evangelizo Reader Feed
...
PrefLang = Literal["FR", "DE", "EN", "ES", "IT", "PT"]
...
def fetch_liturgy_day(
    ...
    *,
    ...
    - ``FR`` → AELF france
    - ``DE`` / ``EN`` / ``ES`` / ``IT`` / ``PT`` → Evangelizo (codes Reader DE / AM / SP / IT / PT)
    """
